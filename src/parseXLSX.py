...
def process_sheet():
    ...
    bar = api.get_bars(list(stock['Symbol']), TimeFrame(1, TimeFrameUnit.Day), start=str(datetime.date.today() - datetime.timedelta(days=1)), end=str(datetime.date.today() - datetime.timedelta(days=1)))
    print(len(bar))
